...
def get_compare_constraint(parent_op,parent_cond, cur_eval, cur_type, node_idx, sub_evaluations, sub_api_terms, solver, model):
    constraint = {"l" : cur_eval, "u": cur_eval}
    if parent_cond == [True,False]:
        return {"l": -inf, "u": inf}
    if parent_op == "<":
            ...
            if node_idx == 0:
                if cur_type == "Real":
                    if solver == "z3":
                        ...
                        constraint = {"l":-inf, "u": model.getValue(model.mkTerm(Kind.SUB,sub_api_terms[0],model.mkReal(1,100000)))}
                    else:
                    ...
                    if solver == "z3":
                        ...
                        constraint = {"l":-inf, "u": model.getValue(model.mkTerm(Kind.SUB,sub_api_terms[0],model.mkInteger(1)))}
                    else:
                ...
                if cur_type == "Real":
                    if solver == "z3":
                        ...
                        constraint = {"l": model.getValue(model.mkTerm(Kind.ADD,sub_api_terms[-1],model.mkReal(1,100000))), "u": inf}
                    else:
                    ...
                    if solver == "z3":
                        ...
                        constraint = {"l": model.getValue(model.mkTerm(Kind.ADD,sub_api_terms[-1],model.mkInteger(1))), "u": inf}
                    else:
                        constraint = {"l":sub_evaluations[-1]+interval(1.0),"u": inf}
            else:
                if cur_type == "Real":
                    if solver == "z3":
                        constraint = {"l":simplify(sub_evaluations[node_idx-1]+1e-6), "u": simplify(sub_evaluations[node_idx]-(1e-6))}
                    elif solver == "cvc":
                        constraint = {"l":model.getValue(model.mkTerm(Kind.ADD,sub_api_terms[node_idx-1],model.mkReal(1,100000))), "u": model.getValue(model.mkTerm(Kind.SUB,sub_api_terms[node_idx],model.mkReal(1,100000)))}
                    else:
                        constraint = {"l":sub_evaluations[node_idx-1]+1e-6, "u": sub_evaluations[node_idx]-1e-6}
                else:
                    if solver == "z3":
                        constraint = {"l":simplify(sub_evaluations[node_idx-1]+1), "u": simplify(sub_evaluations[node_idx]-(1))}
                    elif solver == "cvc":
                        constraint = {"l":model.getValue(model.mkTerm(Kind.ADD,sub_api_terms[node_idx-1],model.mkInteger(1))), "u": model.getValue(model.mkTerm(Kind.SUB,sub_api_terms[node_idx],model.mkInteger(1)))}
                    else:
                        constraint = {"l":sub_evaluations[node_idx-1]+interval(1.0), "u": sub_evaluations[node_idx]-interval(1)}
        else:
            if node_idx == 0:
            ...
            if node_idx == 0:
                if cur_type == "Real":
                    if solver == "z3":
                        ...
                        constraint = {"l": model.getValue(model.mkTerm(Kind.ADD,sub_api_terms[0],model.mkReal(1,100000))), "u": inf}
                    else:
                    ...
                    if solver == "z3":
                        ...
                        constraint = {"l": model.getValue(model.mkTerm(Kind.ADD,sub_api_terms[0],model.mkInteger(1))), "u": inf}
                    else:
                ...
                if cur_type == "Real":
                    if solver == "z3":
                        ...
                        constraint = {"l":-inf, "u": model.getValue(model.mkTerm(Kind.SUB,sub_api_terms[-1],model.mkReal(1,100000)))}
                    else:
                    ...
                    if solver == "z3":
                        ...
                        constraint = {"l":-inf, "u": model.getValue(model.mkTerm(Kind.SUB,sub_api_terms[-1],model.mkInteger(1)))}
                    else:
                        constraint = {"l":-inf, "u": sub_evaluations[-1]-interval(1)}
            else:
                if cur_type == "Real":
                    if solver == "z3":
                        constraint = {"l":simplify(sub_evaluations[node_idx]+1e-6), "u": simplify(sub_evaluations[node_idx-1]-(1e-6))}
                    elif solver == "cvc":
                        constraint = {"l":model.getValue(model.mkTerm(Kind.ADD,sub_api_terms[node_idx],model.mkReal(1,100000))), "u": model.getValue(model.mkTerm(Kind.SUB,sub_api_terms[node_idx-1],model.mkReal(1,100000)))}
                    else:
                        constraint = {"l":sub_evaluations[node_idx]+1e-6, "u": sub_evaluations[node_idx-1]-1e-6}
                else:
                    if solver == "z3":
                        constraint = {"l":simplify(sub_evaluations[node_idx]+1), "u": simplify(sub_evaluations[node_idx-1]-(1))}
                    elif solver == "cvc":
                        constraint = {"l":model.getValue(model.mkTerm(Kind.ADD,sub_api_terms[node_idx],model.mkInteger(1))), "u": model.getValue(model.mkTerm(Kind.SUB,sub_api_terms[node_idx-1],model.mkInteger(1)))}
                    else:
                        constraint = {"l":sub_evaluations[node_idx]+interval(1.0), "u": sub_evaluations[node_idx-1]-interval(1)}
    elif parent_op == ">":
        if parent_cond:
            if node_idx == 0:
                if cur_type == "Real":
...
def get_real_constraint(parent_op,parent_cond, cur_eval, cur_type, node_idx, sub_evaluations, sub_api_terms, solver, model):
    constraint = {"l":cur_eval, "u":cur_eval}
...
def get_int_constraint(parent_op,parent_cond, cur_eval, cur_type, node_idx, sub_evaluations, sub_api_terms, solver, model):
    constraint = {"l":cur_eval, "u":cur_eval}
    return constraint


def get_arith_constraint(parent_op, parent_cond, cur_eval, cur_type, node_idx, sub_evaluations, sub_api_terms, solver, model):
    constraint = {"l":cur_eval, "u":cur_eval}
    if parent_op == "*":
        if solver == "z3":
...
def get_string_constraint(parent_op,parent_cond, cur_eval, cur_type, node_idx, sub_evaluations, sub_api_terms, solver, model):
...
def get_regp_constraint(parent_op,parent_cond, cur_eval, cur_type, node_idx, sub_evaluations, sub_api_terms, solver, model):
