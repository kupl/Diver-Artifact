...
def make_statistics(logs):
    ...
    cores = [[0 for i in range(31)] for i in range(3)]
    min_time = [ datetime(2032,1,1,0,0,0)-datetime.now() for i in range(3)]
    max_time = [ datetime.now()-datetime.now() for i in range(3) ]
    
    number_of_gen = [ 0 for i in range(3)]
    number_of_mutants = [ 0 for i in range(3)]
    sat_time = [0.0 for i in range(3)]
    unsat_time = [0.0 for i in range(3)]
    testing_time = [0.0 for i in range(3)]
    gen_time = [0.0 for i in range(3)]
    judge_time = [0.0 for i in range(3)]

    number_of_sat_mutants = [0 for i in range(3)]
    number_of_unsat_mutants = [0 for i in range(3)]


    number_of_gen_core = [ [0 for j in range(31)] for i in range(3)]
    number_of_mutants_core = [ [0 for j in range(31)] for i in range(3)]

    number_of_sat_mutants_core = [ [0 for j in range(31)] for i in range(3)]
    number_of_unsat_mutants_core = [ [0 for j in range(31)] for i in range(3)]


    sat_time_core = [ [0.0 for j in range(31)] for i in range(3)]
    unsat_time_core = [ [0.0 for j in range(31)] for i in range(3)]
    testing_time_core = [ [0.0 for j in range(31)] for i in range(3)]
    judge_time_core = [ [0.0 for j in range(31)] for i in range(3)]
    gen_time_core = [ [0.0 for j in range(31)] for i in range(3)]
...
def get_bug_status(bugs):
    ...
    for b in bugs:
        ...
        idx = int(bug_info[-2].split('.')[0].split('_')[2])
...
def print_bug_status(status):
    ...
    for idx in range(1,3):
        ...
        if status[idx]=="O":
            ...
            fail_res += 1
    print("|---------------------------------------|")
    print("| #Total: {0:^2} | #Success: {1:^2} | #Fail: {2:^2} |".format(2,str(find_res),str(fail_res)))
    return
...
def print_bug_details_v2(count_bugs, infos,start,end):
    ...
    print("|--------------------------------------------------------------------------------------------------------------------------------------------------|")
    print("| {0:^13} | {1:^12} | {2:^8} | {3:^8} | {4:^9} | {5:^9} | {6:^7} | {7:^7} | {8:^7} | {9:^12} | {10:^8} | {11:^10} |".format("Bench_ID","# Reproduced", "Min_Time","Max_Time","# Mutants", "# Gen", "Rate","# SAT","# UNSAT","TESTING_TIME","GEN_TIME","JUDGE_TIME"))
    print("|--------------------------------------------------------------------------------------------------------------------------------------------------|")
    ...
    for idx in range(start,end+1):
        ...
        print("| {0:^13} | {1:^12} | {2:^8} | {3:^8} | {4:^9} | {5:^9} | {6:^7} | {7:^7} | {8:^7} | {9:^12} | {10:^8} | {11:^10} |".format(benchmark,reproduced,min_t,max_t,mutants,gen,rate,sat,unsat,st,ut,tt,gt,jd))
    print("|-----------------------------------------------------------------------------------------------------------------------------------------------|")
    return
...
def main():
    ...
    if detail == "true":
        ...
        #if args.bench == "ALL":
        print_bug_details_v2(count_bugs,infos,args.start,args.end)
        #else:
        #    print_bug_details_bench(count_bugs,core_infos)
    else:
